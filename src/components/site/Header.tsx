  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-white/60 bg-white/82 shadow-[0_10px_40px_-30px_rgba(42,37,43,0.35)] backdrop-blur-2xl supports-[backdrop-filter]:bg-white/72">
      <div className="mx-auto flex h-20 max-w-[1280px] items-center justify-between px-6">
        <Link to="/" aria-label="Księgowa360 — strona główna" className="transition-transform duration-300 hover:-translate-y-0.5">
          <Logo />
        </Link>
 {/*
        <nav className="hidden items-center gap-2 rounded-full border border-border bg-white/70 px-2 py-2 shadow-soft lg:flex" aria-label="Główna nawigacja">
          {nav.map((n) => (
            <Link
              key={n.to}
              to={n.to}
              ...
              inactiveProps={{ className: "text-dark-soft hover:bg-soft-bg hover:text-foreground" }}
